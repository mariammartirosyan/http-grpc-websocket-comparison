...
apply_kubectl "../gRPC/Manifest Files/account-db.yaml"

echo "Deploying the movie database"
apply_kubectl "../gRPC/Manifest Files/movie-db.yaml"

echo "Deploying the statistics database"
apply_kubectl "../gRPC/Manifest Files/statistics-db.yaml"
...
kubectl wait --for='jsonpath={.status.conditions[?(@.type=="Ready")].status}=True' deployment.apps/account-service-grpc-deployment --timeout=100s
